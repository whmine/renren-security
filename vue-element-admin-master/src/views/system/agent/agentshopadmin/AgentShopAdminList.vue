<template>
    <div class="app-container">
        <div class="filter-container">
            <router-link v-permission="['agent:agentshopadmin:save']" :to="'/agentResource/AgentShopAdminCreate'">
        ...
        >
            ...
            <el-table-column width="280px" align="center" label="管理员编码">
                <template slot-scope="scope">
                    <span>{{ scope.row.adminSn }}</span>
                </template>
            </el-table-column>
                <el-table-column width="280px" align="center" label="管理员名称">
                <template slot-scope="scope">
                    <span>{{ scope.row.adminName }}</span>
                </template>
            </el-table-column>
                <el-table-column width="280px" align="center" label="管理员手机">
                <template slot-scope="scope">
            ...
            <el-table-column fixed="right" align="left" label="操作" width="120px">
                <template slot-scope="scope">
                    <router-link v-permission="['agent:agentshopadmin:update']" :to="'/agentResource/AgentShopAdminEdit/'+scope.row.id">
                        <el-tooltip class="item" effect="dark" content="编辑" placement="bottom">
                            <el-button  type="primary" size="small" icon="el-icon-edit" />
                        </el-tooltip>
                    </router-link>
                    <el-tooltip class="item" v-permission="['agent:agentshopadmin:delete']" effect="dark" content="删除" placement="bottom">
                        <el-button
                                size="small"
                                type="primary"
                                icon="el-icon-delete"
                                @click="deleteAgentShopAdmin(scope.row.id)"
                        />
                    </el-tooltip>
                </template>
            </el-table-column>
...
<script>
    ...
    import { findByListAgentShopAdmin, delAgentShopAdmin } from "@/api/agent/AgentShopAdminApi";
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                ...
                findByListAgentShopAdmin(this.listQuery).then(response => {
            ...
            deleteAgentShopAdmin(data) {
                delAgentShopAdmin({ id: data }).then(response => {
                    this.getList();
                });
            }
        }
