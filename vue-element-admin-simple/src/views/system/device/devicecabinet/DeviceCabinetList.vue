<template>
    <div class="app-container">
        <div class="filter-container">
            <router-link v-permission="['device:devicecabinet:save']" :to="'/deviceResource/DeviceCabinetCreate'">
        ...
        >
            ...
            <el-table-column width="280px" align="center" label="机柜编码">
                ...
                <el-table-column width="280px" align="center" label="设备型号">
                <template slot-scope="scope">
                    <span>{{ scope.row.deviceType }}</span>
                </template>
            </el-table-column>
                <el-table-column width="280px" align="center" label="充电槽数量">
                <template slot-scope="scope">
                ...
                <template slot-scope="scope">
                    <span>{{ scope.row.bus }}</span>
                </template>
            </el-table-column>
                <el-table-column width="280px" align="center" label="所属商户">
                <template slot-scope="scope">
                    <span>{{ scope.row.pos }}</span>
                </template>
            </el-table-column>
                <el-table-column width="280px" align="center" label="是否可以租借">
                <template slot-scope="scope">
                    <span>{{ scope.row.lease }}</span>
                </template>
            </el-table-column>
                <el-table-column width="280px" align="center" label="设备状态">
                <template slot-scope="scope">
            ...
            <el-table-column fixed="right" align="left" label="操作" width="120px">
                <template slot-scope="scope">
                    <router-link v-permission="['device:devicecabinet:update']" :to="'/deviceResource/DeviceCabinetEdit/'+scope.row.id">
                        <el-tooltip class="item" effect="dark" content="编辑" placement="bottom">
                            <el-button  type="primary" size="small" icon="el-icon-edit" />
                        </el-tooltip>
                    </router-link>
                    <el-tooltip class="item" v-permission="['device:devicecabinet:delete']" effect="dark" content="删除" placement="bottom">
                        <el-button
                                size="small"
                                type="primary"
                                icon="el-icon-delete"
                                @click="deleteDeviceCabinet(scope.row.id)"
                        />
                    </el-tooltip>
                </template>
            </el-table-column>
...
<script>
    ...
    import { findByListDeviceCabinet, delDeviceCabinet } from "@/api/device/DeviceCabinetApi";
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                ...
                findByListDeviceCabinet(this.listQuery).then(response => {
            ...
            deleteDeviceCabinet(data) {
                delDeviceCabinet({ id: data }).then(response => {
                    this.getList();
                });
            }
        }
